...
fetchProduct()
  ...
  .then((data) => {
    ...
    cart.forEach((item) => {
      ...
      if (!item.id) {
        console.warn(`Product with title "${item.title}" has no ID and will be skipped.`);
        return;
      }

      const productRow = document.createElement("tr");
      productRow.innerHTML = `
        <td class="list-img"><img src="${item.images?.[0] || item.thumbnail || 'https://via.placeholder.com/60'}" alt="${item.title || 'No title'}" /></td>
        ...
        <td><button class="add-to-cart" data-id="${item.id}">Add to Cart</button></td>
      `;
    ...
    document.querySelectorAll(".add-to-cart").forEach((button) => {
      button.addEventListener("click", (e) => {
        ...
        try {
          ...
          console.log(`Added product ${product.title} to cart. Current cart:`, cartItems);
